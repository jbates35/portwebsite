		...
		if(json!="Nothing found.") {
			...
			for(var i=0; i<=vidCount; i++) {
				if(json[i].id == this_id) {
					writeVideo(json[i].id , json[i].title , json[i].ylink , json[i].description , json[i].subject, json[i].course , json[i].chapter , json[i].question , json[i].show);
				}
			}
		}
	});
		...
		if(json!="Nothing found.") {
			...
			for(var m=0; m<=projCount; m++) {
				if(json[m].id == this_id) {
					writeProjectHTML();
					writeProject(json[m].date, json[m].description, json[m].title, json[m].ylink, json[m].creator, json[m].planguage, json[m].id, json[m].olink1, json[m].olink2, json[m].olink3, json[m].olinkdesc1, json[m].olinkdesc2, json[m].olinkdesc3, json[m].imgfilesuploaded, json[m].imgdesc_1, json[m].imgdesc_2, json[m].imgdesc_3, json[m].imgdesc_4, json[m].imgdesc_5, json[m].imgdesc_6, json[m].filecount, json[m].show);
...
function writeVideo(id, title, ylink, description, subjectid, course, chapter, question, show) {
	...
	$('#vidContentContainer_id').append(`
			<div class="vidTitleDiv" id="vidTitleDiv_id`+id+`">
				<h3>(`+subject+`) `+title+`</h3>
			</div>
			<div class="vidYoutubeDiv" id="vidYoutubeDiv_id`+id+`">
				<iframe id="ytframe_id" class="vidYtframe" src="https://www.youtube.com/embed/`+ylink+`" allowfullscreen>
				</iframe>
			</div>
			<div class="vidCourseDiv" id="vidCourseDiv_id`+id+`">
				<b>Course:</b> `+course+`<br>
				<b>Chapter:</b> `+chapter+`<br>
				<b>Question:</b> `+question+`<br>
			</div>
			<div class="vidDescriptionDiv" id="vidDescriptionDiv_id`+id+`">
...
function writeProject(date, description, title, ylink, creator, planguage, id, olink1, olink2, olink3, olinkdesc1, olinkdesc2, olinkdesc3, imgfilesuploaded, imgdesc_1, imgdesc_2, imgdesc_3, imgdesc_4, imgdesc_5, imgdesc_6, filecount, show) {
...
//Content box description string (dString)
	...
	dString += `<label>Programming languages used:</label> ` + planguage + ` <br><br>`;
...
//Filebox code string (fString)
	...
	if(filecount==0) {
		...
		if(olink1) fString += '<li><a href="uploads/project/'+window.currentId+'/'+olink1+'">'+olinkdesc1+'</a></li>';
		if(olink2) fString += '<li><a href="uploads/project/'+window.currentId+'/'+olink2+'">'+olinkdesc2+'</a></li>';
		if(olink3) fString += '<li><a href="uploads/project/'+window.currentId+'/'+olink3+'">'+olinkdesc3+'</a></li>';
		fString += '</ul>';		
	}
