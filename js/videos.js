...
function listVideos() {
	$.getJSON("vid_json.php", function(json) {
		if(json!="Nothing found.") {
			var vidCount=json.length;
			resetVideo();
			if(!window.courseSelect) {
				for(var i=0; i<=vidCount; i++) {
					writeVideo(i, json[i].id , json[i].title , json[i].ylink , json[i].description , json[i].subject, json[i].course , json[i].chapter , json[i].question); 
					if(i>0) {
...
function writeVideo(id_i, id, title, ylink, description, subjectid, course, chapter, question) {
	var subject;
