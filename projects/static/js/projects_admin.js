var jsonFile = "proj_json_admin.php";
...
$(document).ready(function() {
	...
	$.getJSON(jsonFile, function(json) {
		...
		for(m=0; m<window.proj_count; m++) {
			listProject(m, json[m].date, json[m].title, json[m].id, json[m].show);
		}
...
function changeProject(n) {
	m=n;
	$.getJSON(jsonFile, function(json) {
		projectDisplay(json[m].date, json[m].description, json[m].title, json[m].ylink, json[m].creator, json[m].planguage, json[m].id, json[m].olink1, json[m].olink2, json[m].olink3, json[m].olinkdesc1, json[m].olinkdesc2, json[m].olinkdesc3, json[m].imgfilesuploaded, json[m].imgdesc_1, json[m].imgdesc_2, json[m].imgdesc_3, json[m].imgdesc_4, json[m].imgdesc_5, json[m].imgdesc_6, json[m].filecount, json[m].show);
	});
}

//function to make the flexbox project list
function listProject(id_i, date, title, id, show) {

//listing made per project
	$('#mainListingCont_id').append(`
	<div class="box-1">
		<div class="img-box-1">
			<div class="img-click" onclick="changeProject(`+id_i+`); popupProject();">
				<img class="displaypic" id="displayPic_id`+id_i+`" src="">
				<div id="displayTitle_id`+id_i+`" class="title-1">`+title+` <span class="titleyear">(`+date.substring(0,4)+`)<span></div>
			</div>
			<div class="edit-box">
				<button class="eButton" onclick="editProj(`+id+`)">Edit</button>|<button class="eButton" onclick="deleteProj(`+id+`)">Delete</button>
			</div>
		</div>
	</div>
	`);
...
//make sure the display pic exists
	...
	if(show==false) {
		$('#displayTitle_id'+id_i).addClass("showFalseLTitle");
		$('#displayPic_id'+id_i).addClass("showFalseImage");
	}
...
function projectDisplay(date, description, title, ylink, creator, planguage, id, olink1, olink2, olink3, olinkdesc1, olinkdesc2, olinkdesc3, imgfilesuploaded, imgdesc_1, imgdesc_2, imgdesc_3, imgdesc_4, imgdesc_5, imgdesc_6, filecount, show) {
...
//Filebox code string (fString)
	...
	if(filecount==0) {
		...
		if(olink1) fString += '<li><a href="uploads/project/'+id+'/'+olink1+'">'+olinkdesc1+'</a></li>';
		if(olink2) fString += '<li><a href="uploads/project/'+id+'/'+olink2+'">'+olinkdesc2+'</a></li>';
		if(olink3) fString += '<li><a href="uploads/project/'+id+'/'+olink3+'">'+olinkdesc3+'</a></li>';
		fString += '</ul>';		
	}

//actual function:
	$('#projContTitle_id').html(title+` (`+date.substring(0,4)+`) <button class="eButton_proj" onclick="editProj(`+id+`)">Edit</button>|<button class="eButton_proj" onclick="deleteProj(`+id+`)">Delete</button>`); //Title
...
//Image gallery
	...
	//see if there are images uploaded in the first place
		if(imgfilesuploaded==0) {
			...
			`);
			if($('#image-container-id').hasClass('shownone') == false) $('#image-container-id').addClass('shownone');
		} else {
			$('#imgError').html(``);
			if($('#image-container-id').hasClass('shownone') == true) $('#image-container-id').removeClass('shownone');
			$('#image-holder-img-id').attr('src', 'uploads/project/'+window.currentId+'/img/large/1.jpg');
		}
	...
	//set preview image srcs and displays
		...
		$('#img-prev-div_'+o).removeClass('showblock');
			if(o<=imgfilesuploaded) {
				$('#img-prev-src_'+o).attr('src', 'uploads/project/'+id+'/img/small/'+o+'.jpg');
	...
	//set image desc array
		descArray = [ imgdesc_1, imgdesc_2, imgdesc_3, imgdesc_4, imgdesc_5, imgdesc_6 ];
	...
	if(show==true) {
		if($('#project-container-overlay_id').hasClass("showFalseCont"))
			$('#project-container-overlay_id').removeClass("showFalseCont");
	} else {
		if(!$('#project-container-overlay_id').hasClass("showFalseCont"))
		$('#project-container-overlay_id').addClass("showFalseCont");
	}
...
function changeImage(n) {
	...
	$('#image-holder-img-id').attr('src', 'uploads/project/'+window.currentId+'/img/large/'+window.imageSelect+'.jpg');
	if(imageDownB==true) {
		changeImageDesc(n);
	}
}
...
function editProj(id) {
	window.location.href="./edit.php?type=project&id="+id;
}

function deleteProj(id) {
	window.location.href="./delete.php?type=project&id="+id;
}	
